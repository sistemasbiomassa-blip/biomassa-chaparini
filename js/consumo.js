// ==================== CONSUMO ====================
var consumoFiltro={motorista:'',placa:'',localCarga:'',mes:'',dtIni:'',dtFim:''};
...
function buildConsumo(){
  ...
  var uMot=[],uPla=[],uMes=[],uLocalCarga=[];
  var motSet={},plaSet={},mesSet={},localCargaSet={};
  DB.cadastro.forEach(function(r){
    ...
    if(r['LOCAL CARGA']&&!localCargaSet[r['LOCAL CARGA']]){localCargaSet[r['LOCAL CARGA']]=1;uLocalCarga.push(r['LOCAL CARGA'])}
  });
  uMot.sort();uPla.sort();uMes.sort();uLocalCarga.sort();
  ...
  createFilters('filtersConsumo',[
    ...
    {id:'fcLocalCarga',label:'Local de Carga',options:uLocalCarga,onChange:onConsumoFilterChange},
  ...
  if(document.getElementById('fcLocalCarga')) document.getElementById('fcLocalCarga').value=consumoFiltro.localCarga||'';
...
function onConsumoFilterChange(){
  ...
  consumoFiltro.localCarga=document.getElementById('fcLocalCarga')?document.getElementById('fcLocalCarga').value:'';
...
// Placas que fizeram pelo menos 1 viagem a partir do local de carga informado, no período
// filtrado (mês OU intervalo de datas). Usado pra restringir o gauge e a tabela por placa
// às placas "dessa rota" — não isola o km/L exato da rota (ver conversa com o usuário: um
// trecho entre abastecimentos pode incluir dias sem carga registrada, então não dá pra
// atribuir o consumo com precisão a um local específico).
function calcPlacasPorLocalCarga(localCarga, mesFiltro, dtIniF, dtFimF){
  var hasRange=!!(dtIniF||dtFimF);
  var set={};
  DB.cadastro.forEach(function(r){
    if(r['LOCAL CARGA']!==localCarga) return;
    if(!r.PLACA) return;
    if(hasRange){
      if(!dateInRange(r.DATA,dtIniF,dtFimF)) return;
    } else if(mesFiltro && getMonthYear(r.DATA)!==mesFiltro){
      return;
    }
    set[String(r.PLACA).replace(/\s+/g,'')]=1;
  });
  return set;
}
...
function calcConsumoPlacasT2T(){
  var f=consumoFiltro;
  var hasRangeC=!!(f.dtIni||f.dtFim);
  var placasLocalCarga=f.localCarga?calcPlacasPorLocalCarga(f.localCarga,f.mes,f.dtIni,f.dtFim):null;
  var rowsPlaca=DB.cadastro.filter(function(r){
    ...
    if(placasLocalCarga){
      var pl=r.PLACA?String(r.PLACA).replace(/\s+/g,''):'';
      if(!placasLocalCarga[pl]) return false;
    }
    return true;
  });
...
function renderConsumoPlacas(){
  ...
  if(consumoFiltro.localCarga){
    h+='<div style="padding:8px 14px;background:var(--surface2);border-bottom:1px solid var(--border);font-size:11px;color:var(--text2)">ℹ️ Mostrando as placas que carregaram em <strong>'+consumoFiltro.localCarga+'</strong> no período — o km/L é o consumo geral de cada placa, não isolado só dessa rota.</div>';
  }
  if(suspeitas>0){
...
function renderConsumoAbastList(){
  ...
  var placasLocalCargaA=f.localCarga?calcPlacasPorLocalCarga(f.localCarga,f.mes,f.dtIni,f.dtFim):null;
  var rows=DB.cadastro.filter(function(r){
    ...
    if(placasLocalCargaA){
      var pl=r.PLACA?String(r.PLACA).replace(/\s+/g,''):'';
      if(!placasLocalCargaA[pl]) return false;
    }
    return true;
...
function exportConsumoPDF(){
  ...
  if(f.localCarga) filtrosTxt.push('Local de Carga: '+f.localCarga);
  if(f.dtIni || f.dtFim){
  ...
  var placasLocalCargaE=f.localCarga?calcPlacasPorLocalCarga(f.localCarga,f.mes,f.dtIni,f.dtFim):null;
  var abast=DB.cadastro.filter(function(r){
    ...
    if(placasLocalCargaE){
      var pl=r.PLACA?String(r.PLACA).replace(/\s+/g,''):'';
      if(!placasLocalCargaE[pl]) return false;
    }
    return true;
  }).sort(function(a,b){return(b.DATA||'').localeCompare(a.DATA||'')});
