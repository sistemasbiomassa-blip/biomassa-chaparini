...
function _pdfPrepararClone(clone){
  ...
  clone.querySelectorAll('table').forEach(function(tb){
    var idx=-1;
    tb.querySelectorAll('thead th').forEach(function(th,i){ if(th.textContent.trim().toLowerCase()==='ações') idx=i; });
    if(idx<0) return;
    tb.querySelectorAll('tr').forEach(function(tr){ var c=tr.children[idx]; if(c) c.parentNode.removeChild(c); });
  });
  ...
  // a busca de pneu some junto com o quadro dela; os demais campos/botões, só eles
  var busca=clone.querySelector('#pneuBuscaInput');
  if(busca){ var cx=busca.closest('.table-container'); if(cx) cx.parentNode.removeChild(cx); }
  clone.querySelectorAll('input,button,.manut-view-tabs').forEach(function(el){ el.parentNode.removeChild(el); });
}

// Exporta SÓ uma seção (um quadro/tabela), em vez da página inteira. O botão genérico dos
// filtros chama exportPagePDF, que clona a página toda — no Detalhamento de Manutenção
// isso levava junto a matriz, o painel de KM e os cards.
function exportSecaoPDF(elId,titulo,filtrosTxt,paisagem){
  var el=document.getElementById(elId);
  if(!el){ showToast('Nada para exportar',true); return; }
  // abre a janela já no clique (gesto do usuário), senão o navegador bloqueia em silêncio
  var printWin=window.open('','_blank');
  if(!printWin){ showToast('Seu navegador bloqueou a janela do PDF. Permita pop-ups para este site.',true); return; }
  var clone=el.cloneNode(true);
  _pdfPrepararClone(clone);
  var brd='#cccccc', agora=new Date();
  var html='<!DOCTYPE html><html><head><meta charset="UTF-8"><title>'+titulo+' - Biomassa Chaparini</title>';
  ...
  html+='body{font-family:"Barlow",sans-serif;background:#ffffff;color:#111111;padding:20px 26px}';
  html+='.pdf-header{display:flex;justify-content:space-between;align-items:center;padding-bottom:10px;border-bottom:2px solid '+brd+';margin-bottom:12px;background:#ffffff}';
  html+='.pdf-filtros{font-size:10px;color:#555555;margin-bottom:10px}';
  html+='.table-header,.manut-ph{display:flex;justify-content:space-between;align-items:center;margin-bottom:8px}';
  html+='.table-header h3,.manut-ph-title{font-family:"Barlow Condensed",sans-serif;font-size:15px;font-weight:600}';
  html+='.chart-badge,.manut-ph-badge{font-size:9px;color:#555555;border:1px solid '+brd+';border-radius:20px;padding:2px 8px}';
  ...
  html+='.maq-chip,.manut-chip{font-size:8.5px;border:1px solid '+brd+';border-radius:20px;padding:1px 6px}';
  html+='.manut-chip-dot{display:none}';
  html+='@media print{body{padding:8px}@page{size:'+(paisagem?'landscape':'portrait')+';margin:18mm 8mm 8mm 8mm}.pdf-header{position:fixed;top:0;left:0;right:0}}';
  html+='</style></head><body>';
  html+='<div class="pdf-header"><div style="display:flex;align-items:center;gap:12px">';
  html+='<img src="'+PDF_LOGO+'" style="height:40px;width:auto;object-fit:contain" alt="Biomassa Chaparini">';
  html+='<div><div style="font-family:Barlow Condensed,sans-serif;font-size:16px;font-weight:700;color:#1a4a2e;letter-spacing:0.5px">BIOMASSA CHAPARINI</div>';
  html+='<div style="font-size:9px;color:#555555">Sistema de Gestão de Frota</div></div></div>';
  html+='<div style="text-align:right"><div style="font-family:Barlow Condensed,sans-serif;font-size:13px;font-weight:600">'+titulo+'</div>';
  html+='<div style="font-size:8px;color:#555555;margin-top:2px">Gerado em '+agora.toLocaleDateString('pt-BR')+' às '+agora.toLocaleTimeString('pt-BR')+'</div></div></div>';
  if(filtrosTxt) html+='<div class="pdf-filtros"><b>Filtros aplicados:</b> '+filtrosTxt+'</div>';
